...
#include "xdr/SuperBlockChain-SCP.h"
...
void
HerderSCPDriver::restoreSCPState(uint64_t index, SuperBlockChainValue const& value)
{
...
HerderSCPDriver::validateValueHelper(uint64_t slotIndex,
                                     SuperBlockChainValue const& b) const
{
                                   ...
                                   std::set<Value> const& candidates)
{
    ...
    SuperBlockChainValue comp(h, 0, emptyUpgradeSteps, 0);
    ...
    std::vector<SuperBlockChainValue> candidateValues;

    for (auto const& c : candidates)
    {
        candidateValues.emplace_back();
        SuperBlockChainValue& sv = candidateValues.back();
...
HerderSCPDriver::valueExternalized(uint64_t slotIndex, Value const& value)
{
    ...
    catch (...)
    {
        ...
        // therefore contain a valid SuperBlockChainValue.
        CLOG(ERROR, "Herder") << "HerderSCPDriver::valueExternalized"
                              << " Externalized SuperBlockChainValue malformed";
...
void
HerderSCPDriver::nominate(uint64_t slotIndex, SuperBlockChainValue const& value,
                          TxSetFramePtr proposedSet,
                          SuperBlockChainValue const& previousValue)
{
