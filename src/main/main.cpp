...
static const struct option snb_core_options[] = {
...
usage(int err = 1)
{
    ...
    os << "usage: snb-core [OPTIONS]\n"
          ...
          "      --forcescp           Next time snb-core is run, SCP will "
          "start "
    ...
    if (checkInitialized(app))
    {
        ...
        catch (std::invalid_argument const&)
        {
            ...
            LOG(INFO) << "* If you really want to catchup to " << to
                      << " run snb-core with --newdb parameter.";
            LOG(INFO) << "*";
            return;
        }
...
{
    LOG(INFO) << "Starting snb-core " << STELLAR_CORE_VERSION;
...
main(int argc, char* const* argv)
{
    ...
    std::string cfgFile("snb-core.cfg");
    std::string command;
    ...
    int opt;
    while ((opt = getopt_long_only(argc, argv, "c:", snb_core_options,
                                   nullptr)) != -1)
    {
        switch (opt)
        {
