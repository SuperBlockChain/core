...
The `get` and `put` commands define subprocesses that `snb-core` will
...
called `s3snb` based on the [AWS S3](https://aws.amazon.com/s3/) storage system:

~~~~
[HISTORY.s3snb]
get="curl http://history.snb.org/{0} -o {1}"
put="aws s3 cp {0} s3://history.snb.org/{1}"
~~~~

In this example, `snb-core` will use the `curl` command to fetch history and
