...
class Floodgate
{
    class FloodRecord
    {
      public:
        ...
        uint32_t mLedgerSeq;
        SuperBlockChainMessage mMessage;
        std::set<Peer::pointer> mPeersTold;

        FloodRecord(SuperBlockChainMessage const& msg, uint32_t ledger,
                    Peer::pointer peer);
    };
  ...
  public:
    ...
    bool addRecord(SuperBlockChainMessage const& msg, Peer::pointer fromPeer);

    void broadcast(SuperBlockChainMessage const& msg, bool force);
