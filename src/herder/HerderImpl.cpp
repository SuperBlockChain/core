...
void
HerderImpl::valueExternalized(uint64 slotIndex, SuperBlockChainValue const& value)
{
    updateSCPCounters();
    trackingHeartBeat();
...
HerderImpl::triggerNextLedger(uint32_t ledgerSeqToTrigger)
{
    ...
    SuperBlockChainValue newProposedValue(txSetHash, nextCloseTime, emptyUpgradeSteps,
                                  0);
    ...
    std::vector<TransactionFramePtr> const& applied)
{
    ...
    // propagation
    {
        ...
        {
            auto msg = tx->toSuperBlockChainMessage();
