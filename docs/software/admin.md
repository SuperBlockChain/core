...
SuperBlockChain Core is responsible for communicating directly with and maintaining 
the SuperBlockChain peer-to-peer network. For a high-level introduction to SuperBlockChain Core, [watch this talk](https://www.youtube.com/watch?v=pt_mm8S9_WU) on the architecture and ledger basics:

[![Introduction to SuperBlockChain Core](https://i.ytimg.com/vi/pt_mm8S9_WU/hqdefault.jpg "Introduction to SuperBlockChain Core")](https://www.youtube.com/watch?v=pt_mm8S9_WU)

It will also be useful to understand how [data flows](https://www.snb.org/developers/snb-core/software/core-data-flow.pdf) and is stored in snb-core.

## Why run a node?

Run snb-core if you want to:
* Obtain the most up-to-date and reliable data from the SuperBlockChain network
* Generate extended meta data on activity within the SuperBlockChain network (change tracking, etc)
* Submit transactions and their confirmations without depending on a third party
* Extended control on which parties to trust in the SuperBlockChain network
* Participate in validating the SuperBlockChain network


## Building
See [readme](https://github.com/snb/snb-core/blob/master/README.md) for build instructions.
We also provide a [docker container](https://github.com/snb/docker-snb-core-horizon) for a potentially quicker set up than building from source.

## Configuring
All configuration for snb-core is done with a TOML file. By default 
snb-core loads 
...
`$ snb-core --conf betterfile.cfg` 

The [example config](https://github.com/snb/snb-core/blob/master/docs/snb-core_example.cfg) describes all the possible 
configuration options.  

Here is an [example test network config](https://github.com/snb/docker-snb-core-horizon/blob/master/testnet/core/etc/snb-core.cfg) for connecting to the test network.

Here is an [example public network config](https://github.com/snb/docs/blob/master/other/snb-core-validator-example.cfg) for connecting to the public network.

The examples in this file don't specify `--conf betterfile.cfg` for brevity.

## Running
SuperBlockChain-core can be run directly from the command line, or through a supervision 
system such as `init`, `upstart`, or `systemd`.

SuperBlockChain-core sends logs to standard output and `snb-core.log` by default, 
...
`$ snb-core -c "ll?level=debug&partition=history"` 
...
SuperBlockChain-core can be gracefully exited at any time by delivering `SIGINT` or
...
SuperBlockChain-core can also be packaged in a container system such as Docker, so long 
as `BUCKET_DIR_PATH`, `TMP_DIR_PATH`, and the database are stored on persistent 
volumes. For an example, see [docker-snb-core](https://github.com/snb/docker-snb-core-horizon).

Note: `BUCKET_DIR_PATH` and `TMP_DIR_PATH` *must* reside on the same volume
as snb-core needs to rename files between the two.

## Administrative commands
While running, interaction with snb-core is done via an administrative 
...
or by a mid-tier application to submit transactions to the SuperBlockChain network. 
...
Currently snb-core requires very modest hardware. It would be fine to run 
...
See a [list of other validators](https://github.com/snb/docs/blob/master/validators.md).

## Database
SuperBlockChain-core stores the state of the ledger in a SQL database. This DB should 
...
see [example config](https://github.com/snb/snb-core/blob/master/docs/snb-core_example.cfg).

When running snb-core for the first time, you must initialize the database:
...
## Buckets
SuperBlockChain-core stores a duplicate copy of the ledger in the form of flat XDR files 
...
can be ignored--they are managed by snb-core, but they should be stored on 
a fast local disk with sufficient space to store several times the size of the 
current ledger. 

## History archives
SuperBlockChain-core normally interacts with one or more "history archives," which are 
...
The [example config](https://github.com/snb/snb-core/blob/master/docs/snb-core_example.cfg) 
shows how to configure a history archive through command templates. 

While it is possible to run a snb-core node with no configured history 
...
`$ snb-core --newhist <historyarchive>`
...
## A brief explanation
An important distinction in SuperBlockChain compared to traditional quorum based
...
1. `$ snb-core --newhist <historyarchive>`
...
```
   "info" : {
      ...
      "network" : "Public Global SuperBlockChain Network ; September 2015",
      "numPeers" : 12,
      "protocol_version" : 1,
      "quorum" : {
         "474366" : {
...
`$ snb-core --c 'quorum?node=$sdf1` or `$ snb-core --c 'quorum?node=@GABCDE` 
...
of this syncing time is simply snb-core waiting for the next history 
...
* [architecture.md](https://github.com/snb/snb-core/blob/master/docs/architecture.md) 
  describes how snb-core is structured internally, how it is intended to be 
