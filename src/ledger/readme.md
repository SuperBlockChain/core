# What is a ledger?
A ledger represents the state of the SuperBlockChain universe at a given point in time.
...
[`src/xdr/SuperBlockChain-ledger.x`](../xdr/SuperBlockChain-ledger.x)
...
During consensus, nodes work together to decide on the value of SuperBlockChainValue.
SuperBlockChainValue is then saved in the scpValue field of the ledger header.
Any node on the network, given the previous ledger (their previous state) and
'SuperBlockChainValue' should be able to transition to the same new ledger.
...
[`src/xdr/SuperBlockChain-ledger-entries.x`](../xdr/SuperBlockChain-ledger-entries.x)

### AccountEntry
This entry represents an account. In SuperBlockChain, everything is centered around
...
Offers are a way to automate simple trading inside the SuperBlockChain network.
