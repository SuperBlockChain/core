...
getTestConfig(int instanceNumber, Config::TestDbMode mode)
{
    ...
    {
        gTestRoots.emplace_back("snb-core-test");

        std::string rootDir = gTestRoots.back().getName();
        rootDir += "/";

        cfgs[instanceNumber] = snb::make_unique<Config>();
        Config& thisConfig = *cfgs[instanceNumber];

        std::ostringstream sstream;

        sstream << "snb" << instanceNumber << ".log";
     ...
     std::vector<std::string> const& metrics)
{
    ...
    LOG(INFO) << "Testing snb-core " << STELLAR_CORE_VERSION;
