...
#include "util/asio.h"
#include "SuperBlockChainCoreVersion.h"
 ...
 *     random SuperBlockChainMessages. This is the mode you use to generate seed data
 *     for the external fuzzer's corpus.
 *
 *   - In --fuzz mode it reads back a file and appplies it to a pair of of
 *     snb-cores in loopback mode, cranking the I/O loop to simulate
...
std::string
msgSummary(SuperBlockChainMessage const& m)
{
...
bool
tryRead(XDRInputFileStream& in, SuperBlockChainMessage& m)
{
    try
    {
        return in.readOne(m);
     ...
     std::vector<std::string> const& metrics)
{
    ...
    LOG(INFO) << "Fuzzing snb-core " << STELLAR_CORE_VERSION;
...
genfuzz(std::string const& filename)
{
    ...
    out.open(filename);
    autocheck::generator<SuperBlockChainMessage> gen;
    for (size_t i = 0; i < n; ++i)
    {
        try
        {
            SuperBlockChainMessage m(gen(10));
            out.writeOne(m);
