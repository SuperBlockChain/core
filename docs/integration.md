...
snb-core is at the bottom of the SuperBlockChain stack, many other services can be
 built on top of it.
...
# Integration points
## Data consumers
Many types of services can be built based on the data generated by snb-core.

snb-core generates two types of data that can be queried by applications.

Unless otherwise noted, data stored is `validated`, ie data is only persisted
after consensus is reached on the network.

An overview of how the data flows within snb-core is [this data flow presentation](software/core-data-flow.pdf)
...
properly registered by updating its cursor so that snb-core can safely
garbage collect data no longer needed.
By default snb-core only keeps track of its internal state and will
 ...
 * additional meta data not available in history is produced by snb-core.
 ...
 * does not depend on running snb-core (can be built as a pure client
...
### SuperBlockChain core database

## API to snb-core
  snb-core exposes a simple HTTP based API for performing various
  types of operations on snb-core itself or on entities in the network
