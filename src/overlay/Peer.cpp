...
static std::string
msgSummary(SuperBlockChainMessage const& msg)
{
    switch (msg.type())
    {
    case ERROR_MSG:
...
void
Peer::sendMessage(SuperBlockChainMessage const& msg)
{
    if (Logging::logTrace("Overlay"))
        CLOG(TRACE, "Overlay")
            << "("
...
void
Peer::recvMessage(SuperBlockChainMessage const& snbMsg)
{
    if (shouldAbort())
    {
        return;
    ...
    if (Logging::logTrace("Overlay"))
        CLOG(TRACE, "Overlay")
            ...
            << mApp.getConfig().toShortString(
                   mApp.getConfig().NODE_SEED.getPublicKey())
            << ") recv: " << msgSummary(snbMsg)
            << " from:" << mApp.getConfig().toShortString(mPeerID);

    if (!isAuthenticated() && (snbMsg.type() != HELLO) &&
        (snbMsg.type() != AUTH) && (snbMsg.type() != ERROR_MSG))
    {
        CLOG(WARNING, "Overlay") << "recv: " << snbMsg.type()
    ...
    assert(isAuthenticated() || snbMsg.type() == HELLO ||
           snbMsg.type() == AUTH || snbMsg.type() == ERROR_MSG);
...
void
Peer::recvDontHave(SuperBlockChainMessage const& msg)
{
...
void
Peer::recvGetTxSet(SuperBlockChainMessage const& msg)
{
    auto self = shared_from_this();
...
void
Peer::recvTxSet(SuperBlockChainMessage const& msg)
{
...
void
Peer::recvTransaction(SuperBlockChainMessage const& msg)
{
...
void
Peer::recvGetSCPQuorumSet(SuperBlockChainMessage const& msg)
{
...
void
Peer::recvSCPQuorumSet(SuperBlockChainMessage const& msg)
{
...
void
Peer::recvSCPMessage(SuperBlockChainMessage const& msg)
{
...
void
Peer::recvGetSCPState(SuperBlockChainMessage const& msg)
{
...
void
Peer::recvError(SuperBlockChainMessage const& msg)
{
...
void
Peer::recvAuth(SuperBlockChainMessage const& msg)
{
    if (mState != GOT_HELLO)
    {
...
void
Peer::recvGetPeers(SuperBlockChainMessage const& msg)
{
    sendPeers();
}

void
Peer::recvPeers(SuperBlockChainMessage const& msg)
{
