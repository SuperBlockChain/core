...
Floodgate::FloodRecord::FloodRecord(SuperBlockChainMessage const& msg, uint32_t ledger,
                                    Peer::pointer peer)
...
bool
Floodgate::addRecord(SuperBlockChainMessage const& msg, Peer::pointer peer)
{
    if (mShuttingDown)
    {
        return false;
...
void
Floodgate::broadcast(SuperBlockChainMessage const& msg, bool force)
{
    if (mShuttingDown)
    {
        return;
